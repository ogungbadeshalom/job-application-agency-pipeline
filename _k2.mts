...
const c = new pg.Client({host:'localhost',port:5432,user:'jobbids',password:'jobbids',database:'job_bidder'});
await c.connect();
const r = await c.query('SELECT ai_api_key_encrypted e, ai_api_key_nonce n FROM app_config WHERE id=1');
