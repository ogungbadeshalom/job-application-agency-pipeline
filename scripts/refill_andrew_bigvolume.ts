/* BIG-VOLUME refill for Andrew — proxy-gated big boards (Indeed/Glassdoor/
 * ZipRecruiter) through the mubeng rotating proxy on :8899, remote-only.
 * These boards are the untapped volume source (Andrew: greenhouse 409,
 * jobicy 193 builtin 271 vs glassdoor 0 / ziprecruiter 0). Uses JobSpy's
 * proxy path (run_jobspy.py _use_proxy sets proxying for fragile boards).
 *
 * Usage: npx tsx scripts/refill_andrew_bigvolume.ts
 */
...
// Big boards that need the proxy + are far from saturated for Andrew.
const SITES = ['indeed', 'glassdoor', 'ziprecruiter'];
const TERMS = [
  'data engineer',
...
async function main() {
  console.log(`Big-volume proxy refill for Andrew (${PROFILE_ID})`);
  console.log(`  sites=${SITES.join(',')} via MUBENG_PROXY`);
  ...
  console.log(`Next: verify remote via direct fetch if you want verified_remote on these.`);
}
