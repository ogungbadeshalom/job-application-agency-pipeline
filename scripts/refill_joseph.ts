/* Standalone refill for Joseph brimel's active profile — mirrors refill_andrew.ts.
 * Usage: npx tsx scripts/refill_joseph.ts
 */
import { db } from '../db/repo';
import { runJobSpy, dedupeAndMap } from '../lib/scrape';
import { filterJobsByResume } from '../lib/aiJobMatch';

const PROFILE_ID = 'a67e1f18-1b01-4370-8f79-ed1a3be6cc19'; // joseph brimel (active, assigned worker)
const SITES = ['greenhouse', 'builtin', 'jobicy', 'workingnomads', 'linkedin'];
const TERMS = [
  'software engineer', 'fullstack engineer', 'frontend engineer',
  'backend engineer', 'AI engineer', 'ML engineer',
  'platform engineer', 'LLM engineer',
];
...
async function main() {
  console.log(`Scraping for Joseph (${PROFILE_ID})`);
