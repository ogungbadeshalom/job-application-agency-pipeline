/* Non-LinkedIn, remote-only refill for Kenneth Smith (Erry's client).
 * Uses the production runJobSpy + dedupeAndMap + createJobs chain. No LinkedIn.
 * Kenneth = full-stack / AI engineer (his configured scrape_search_terms).
 */
import { db } from '../db/repo';
import { runJobSpy, dedupeAndMap } from '../lib/scrape';
import { filterJobsByResume } from '../lib/aiJobMatch';

const PROFILE_ID = '44e7a4cc-e302-44e6-9c80-63879338ca91'; // Kenneth Smith (Erry's, has resume)
...
async function main() {
  console.log(`[REFILL] Non-LinkedIn remote-only for Kenneth Smith (${PROFILE_ID})`);
